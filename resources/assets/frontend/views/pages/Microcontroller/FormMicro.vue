<template>
  ...
  >
    <div class="mt-6 flex items-center justify-between px-6">
      <h4>{{ Object.entries(this.data).length === 0 ? "Tambah" : "Update" }} Microcontroller</h4>
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if (Object.entries(this.data)._id === null) {
        ...
        let { nama, _id, img, model, connection_type } = JSON.parse(
    ...
    submitData() {
      ...
      // this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.dataId !== null) {
            this.$store.dispatch("dataMicro/updateItem", obj).catch(err => {
              console.error(err);
            });
          } else {
            delete obj.id;
            obj.popularity = 0;
            this.$store.dispatch("dataMicro/addItem", obj).catch(err => {
              console.error(err);
            });
          }
